...
```{r data_manhattan}
manhattan <- read_csv("~/OneDrive/Duke/STA/R/Labs/LN13/manhattan.csv")
```
...
- Population: mice that did while not on treatment
...
```{r practice_2_ci}
ci_mice_died <- get_ci_rad(boot_prop, level = .90)
ci_mice_died
```
